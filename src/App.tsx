...
const DAY_CONFIG = [
  ...
  { label: "Day 5", name: "Cardio", pools: ['cardio'], icon: <Activity className="w-5 h-5 text-gym-accent" />, bg: "bg-white/[0.03]", border: "border-gym-accent/10", text: "text-white" },
  { label: "Day 6", name: "Equipment", pools: ['equipment'], icon: <RotateCw className="w-5 h-5 text-gym-accent" />, bg: "bg-white/[0.03]", border: "border-gym-accent/10", text: "text-white" },
];
...
const PBBlock = ({ 
  ...
  const pb = pbs[exName];
  const isCardio = Object.values(POOLS).flat().some(e => e && e.name === exName && e.pool === 'cardio');
    ...
    <div className="mt-3 p-4 rounded-sm bg-black/55 border border-gym-accent/20">
      {pb && (
        <div className={`text-[10px] text-gym-accent font-bold uppercase tracking-wider flex items-center gap-2 ${showLatest ? 'mb-4' : ''}`}>
          <Trophy className="w-3 h-3 text-gym-accent" /> Peak: <span className="text-gym-accent-light">{pb.bestWeight}{isCardio ? ' mins × ' : 'kg × '}{pb.bestReps}{isCardio ? ' reps' : ''}</span> <span className="opacity-40 text-[9px] ml-1 tracking-normal font-light">({pb.bestDate})</span>
        </div>
      )}
        ...
        <div className="flex items-end justify-between">
          <div className="flex gap-6">
            <div className="flex flex-col">
              <span className="text-[10px] opacity-40 uppercase tracking-widest mb-1">{isCardio ? 'Time' : 'Weight'}</span>
              <div className="flex items-baseline gap-1">
                <span className="text-xl font-light text-white">{pb.lastWeight}</span>
                <span className="text-[10px] text-white/30 uppercase font-medium">{isCardio ? 'mins' : 'kg'}</span>
              </div>
            </div>
            <div className="flex flex-col">
...
export default function App() {
  ...
  const getWorkoutCalories = (workout: any): number => {
    if (!workout) return 0;
    if (workout.estimatedCalories !== undefined) {
      return workout.estimatedCalories;
    }
    
    const userWeight = profile?.weight || 75; // kg
    const userHeight = profile?.height || 175; // cm
    const userSex = profile?.sex || 'other';

    let sexMultiplier = 1.0;
    if (userSex === 'female') sexMultiplier = 0.9;
    if (userSex === 'other') sexMultiplier = 0.95;
    ...
    (workout.sets || []).forEach((set: any) => {
      ...
      if (isCardio) {
        durationMins = set.weight || 0;
        MET = 8.5;
      } else {
        const activeWorkSecs = (set.reps || 10) * 4;
        const restSecs = 60;
        durationMins = (activeWorkSecs + restSecs) / 60;

        MET = isCompound ? 6.0 : 4.0;

        if (userWeight > 0 && set.weight > 0) {
          const ratio = set.weight / userWeight;
          if (ratio > 1.5) MET += 1.5;
          else if (ratio > 1.0) MET += 1.0;
          else if (ratio > 0.5) MET += 0.5;
        }
      }

      const calories = MET * 3.5 * userWeight / 200 * durationMins * sexMultiplier;
  ...
  const getDailyCaloriesData = (): { date: string; calories: number }[] => {
    ...
    archivedWorkouts.forEach(workout => {
      ...
      if (dateStr) {
        dailyMap[dateStr] = (dailyMap[dateStr] || 0) + calories;
      }
    });
      ...
      if (wDoc.exists()) {
        ...
        if (data.days) {
          ...
          if (!Array.isArray(data.days)) {
            for (let i = 0; i < DAY_CONFIG.length; i++) daysArr.push(data.days[`d${i}`] || []);
          } else {
            daysArr = [...(data.days as Exercise[][])];
            while (daysArr.length < DAY_CONFIG.length) {
              daysArr.push([]);
            }
          }
  ...
  const handleSaveSet = async (exName: string, weight: string, reps: string) => {
    ...
    setToast({ 
      message: isNewPB ? `New PB: ${exName}!` : `Logged ${exName}: ${nWeight}${isCardio ? ' mins × ' : 'kg × '}${nReps}${isCardio ? ' reps' : ''}`, 
  ...
  const handleArchiveWorkout = async () => {
    ...
    try {
      ...
      const workoutCalories = Math.round(calculatedCalories);
      ...
      const workoutData = {
        ...
        estimatedCalories: workoutCalories
      };
        ...
        <AnimatePresence mode="wait">
          {dataLoading ? (
                                ...
                                <div key={ex.name} className="bg-black/60 border border-white/10 rounded-sm p-5 hover:border-white/35 transition-all group">
                                  <div className="flex items-center justify-between mb-2">
                                    ...
                                    <div className="flex flex-col flex-1">
                                      <span className="text-[9px] text-white/20 uppercase tracking-widest mb-1 font-bold">{ex.pool === 'cardio' ? 'Time (mins)' : 'Weight'}</span>
                                      <input 
                                        type="number"
                                        inputMode="decimal"
                                        placeholder="---"
            ...
            >
              ...
              <div className="border border-white/15 rounded-sm overflow-hidden bg-black/70 backdrop-blur-md">
                <button 
                  onClick={() => setExpandedProgressSections(prev => ({ ...prev, calories: !prev.calories }))}
                  className="w-full text-left px-8 py-6 flex items-center justify-between hover:bg-[#0c0c0c]/80 transition-colors cursor-pointer group"
                >
                  <div>
                    <h3 className="text-xl font-light italic font-serif flex items-center gap-3 mb-1">
                    ...
                    <p className="text-[10px] text-white/30 uppercase tracking-widest font-bold">Energy expenditure tracking & history</p>
                  </div>
                  <ChevronDown className={`w-5 h-5 text-white/20 group-hover:text-gym-accent transition-all ${expandedProgressSections.calories ? 'rotate-180' : ''}`} />
                </button>

                <AnimatePresence>
                  {expandedProgressSections.calories && (
                    <motion.div
                      initial={{ height: 0, opacity: 0 }}
                      animate={{ height: 'auto', opacity: 1 }}
                      exit={{ height: 0, opacity: 0 }}
                    ...
                    >
                      <div className="px-8 pb-10 pt-4">
                        {/* Summary Stats Grid */}
                        {(() => {
                          const dailyData = getDailyCaloriesData();
                          const totalBurn = dailyData.reduce((sum, d) => sum + d.calories, 0);
                          const totalDays = dailyData.length;
                          const avgBurn = totalDays > 0 ? Math.round(totalBurn / totalDays) : 0;

                          return (
                            <>
                              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8 bg-black/45 border border-white/5 rounded-sm p-4">
                                <div className="p-4 border border-white/5 rounded-sm bg-white/[0.01] flex flex-col justify-between">
                                  <span className="text-[9px] text-white/20 uppercase tracking-widest font-bold font-sans">Total Combined Burn</span>
                                  <div className="mt-2 flex items-baseline gap-2">
                                    <span className="text-3xl font-light text-gym-accent tracking-tight tabular-nums">{totalBurn.toLocaleString()}</span>
                                    <span className="text-[10px] text-white/40 uppercase tracking-widest font-bold">kcal</span>
                                  </div>
                                  <p className="text-[9px] text-white/30 uppercase mt-2">Sum of all archived workouts combined</p>
                                </div>
                                <div className="p-4 border border-white/5 rounded-sm bg-white/[0.01] flex flex-col justify-between">
                                  <span className="text-[9px] text-white/20 uppercase tracking-widest font-bold font-sans">Daily Session Average</span>
                                  <div className="mt-2 flex items-baseline gap-2">
                                    <span className="text-3xl font-light text-gym-accent tracking-tight tabular-nums">{avgBurn.toLocaleString()}</span>
                                    <span className="text-[10px] text-white/40 uppercase tracking-widest font-bold">kcal / day</span>
                                  </div>
                                  <p className="text-[9px] text-white/30 uppercase mt-2">Across {totalDays} unique training days</p>
                                </div>
                              </div>
                              ...
                              <div className="h-[300px] w-full mb-10">
                                {dailyData.length === 0 ? (
                                  <div className="h-full flex flex-col items-center justify-center bg-white/5 rounded-sm border border-white/5 border-dashed">
                                    <Flame className="w-12 h-12 text-white/10 mb-2" />
                                    <p className="text-white/20 font-bold text-sm">Capture workouts to populate calorie progression chart</p>
                                  </div>
                                ) : (
                                  <ResponsiveContainer width="100%" height="100%">
                                    <AreaChart 
                                    ...
                                    >
                                      ...
                                      <Tooltip 
                                        ...
                                        formatter={(value: any) => [`${value} kcal`, 'Calories Burned']}
                                      />
                                      <Area 
                                        type="monotone" 
                                        dataKey="calories" 
                              ...
                              <div className="space-y-4">
                                <h4 className="text-[10px] text-white/40 font-bold uppercase tracking-[0.2em] mb-4">Daily Expenditure Registry</h4>
                                {dailyData.length === 0 ? (
                                  <div className="py-8 text-center text-xs text-white/20 italic bg-white/[0.01] border border-white/5 rounded-sm">
                                    No recorded physical logs archived yet
                                  </div>
                                ) : (
                                  <div className="max-h-80 overflow-y-auto divide-y divide-white/5 border border-white/10 rounded-sm bg-black">
                                    {[...dailyData].reverse().map((entry, idx) => {
                                      const dObj = new Date(entry.date);
                                      const matchingWorkouts = archivedWorkouts.filter(w => w.date === entry.date);
                                      const sessionCount = matchingWorkouts.length;
                                      const totalEx = matchingWorkouts.reduce((sum, w) => sum + (w.exercisesCount || 0), 0);
                                      
                                      return (
                                        <div key={idx} className="flex items-center justify-between px-6 py-4 hover:bg-white/[0.02] transition-colors">
                                          <div className="flex flex-col gap-1">
                                            <span className="text-sm font-semibold text-white/95">
                                              {dObj.toLocaleDateString('en-GB', { weekday: 'long', day: 'numeric', month: 'short', year: 'numeric' })}
                                            </span>
                                            <span className="text-[9px] font-bold text-white/30 uppercase tracking-widest">
                                              {sessionCount} {sessionCount === 1 ? 'Session' : 'Sessions'} • {totalEx} Exercises Completed
                                            </span>
                                          </div>
                                          <div className="flex items-center gap-3">
                                            <div className="flex items-baseline gap-1.5 bg-gym-accent/5 border border-gym-accent/15 px-3 py-1.5 rounded-sm">
                                              <span className="text-base font-light text-gym-accent tabular-nums">{entry.calories}</span>
                                              <span className="text-[8px] text-gym-accent/60 uppercase tracking-widest font-black">kcal</span>
                                            </div>
                                          </div>
                                        </div>
                                      );
                ...
                <div className="space-y-6">
                  ...
                  {sessionSets.length > 0 && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mb-12">
                              ...
                              <div className="space-y-2">
                                {exerciseSets.map((s, idx) => (
                                  <div key={idx} className="flex items-center justify-between group/set">
                                    <div className="flex items-center gap-3">
                                      <span className="text-[11px] tabular-nums text-white/90">{s.weight}{findExerciseByName(name)?.pool === 'cardio' ? ' mins' : 'kg'}</span>
                       ...
                       {/* Calorie Estimation Tracker Card */}
                       {(() => {
                         const userWeight = profile?.weight || 75; // kg
                         const userHeight = profile?.height || 175; // cm
                         const userSex = profile?.sex || 'other';
                         const hasCustomProfile = !!(profile?.weight || profile?.height || profile?.sex);

                         // Multiplier for sex (basal/lean Mass difference)
                         let sexMultiplier = 1.0;
                         if (userSex === 'female') sexMultiplier = 0.9;
                         if (userSex === 'other') sexMultiplier = 0.95;
                         ...
                         const totalBurn = Math.round(totalCalories);
                         const cardioBurn = Math.round(cardioCalories);
                         const strengthBurn = Math.round(strengthCalories);
                         const totalDuration = Math.round(totalEstMins);
                         const trackerTarget = 500; // typical session calorie goal
                         const fillPercent = Math.min(100, Math.max(0, (totalBurn / trackerTarget) * 100));

                         return (
                           <div className="mt-8 p-6 rounded-sm bg-black/65 border border-gym-accent/20 backdrop-blur-md">
                             <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
                               <div className="flex items-start gap-3">
                                 <div className="p-2 bg-gym-accent/10 border border-gym-accent/30 rounded-full mt-1">
                                   <Flame className="w-5 h-5 text-gym-accent animate-pulse" />
                                 </div>
                                 <div>
                                   <h4 className="text-sm font-medium text-white italic font-serif">Energy Expenditure Estimator</h4>
                                   <p className="text-[9px] text-white/30 uppercase tracking-widest font-semibold mt-0.5 animate-pulse">
                                     Live calculation • resting intervals • load density
                                   </p>
                                 </div>
                                </div>
                                <div className="flex items-baseline gap-2 bg-white/5 border border-white/10 px-4 py-2 rounded-sm md:text-right">
                                  <span className="text-2xl font-light text-gym-accent tracking-tight tabular-nums">{totalBurn}</span>
                                  <span className="text-[10px] text-white/40 uppercase tracking-widest font-bold">Estimated kcal</span>
                                </div>
                             </div>
                             ...
                             <div className="space-y-2 mb-6">
                               <div className="flex justify-between items-center text-[9px] text-white/30 uppercase tracking-widest font-bold">
                                 <span>Active Burn Progress</span>
                                 <span>Target: {trackerTarget} kcal</span>
                               </div>
                               <div className="h-4 w-full bg-white/5 rounded-full overflow-hidden border border-white/5 shadow-inner p-[2px]">
                                 <motion.div 
                                   initial={{ width: 0 }}
                                   animate={{ width: `${fillPercent}%` }}
                                   transition={{ duration: 1, ease: "easeOut" }}
                                   className="h-full bg-gradient-to-r from-gym-accent/70 via-gym-accent to-gym-accent-light rounded-full relative"
                                   style={{ boxShadow: '0 0 12px rgba(235,166,45,0.4)' }}
                                 >
                                   {fillPercent > 8 && (
                                     <div className="absolute inset-0 flex items-center justify-end pr-2 text-[8px] font-black text-black/80 tracking-widest uppercase">
                             ...
                             {/* Grid of physiological values & stats */}
                             <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-4 border-t border-white/5 bg-black/35 rounded-sm p-4 text-[10px]">
                               <div className="flex flex-col gap-1 border-r border-white/5">
                                 <span className="text-white/20 uppercase tracking-wider font-bold">Physiology Input</span>
                                 <span className="text-white/85 font-medium flex items-center gap-1">
                                   {hasCustomProfile ? (
                                     <>
                                       {userHeight}cm  •  {userWeight}kg  •  <span className="capitalize">{userSex}</span>
                                     </>
                                   ) : (
                                     <span className="opacity-45">Using Defaults</span>
                                   )}
                                 </span>
                               </div>
                               <div className="flex flex-col gap-1 border-r border-white/5">
                                 <span className="text-white/20 uppercase tracking-wider font-bold">Strength Burn</span>
                                 <span className="text-white/85 font-semibold tabular-nums">{strengthBurn} kcal</span>
                               </div>
                               <div className="flex flex-col gap-1 border-r border-white/5">
                                 <span className="text-white/20 uppercase tracking-wider font-bold">Cardio Burn</span>
                                 <span className="text-white/85 font-semibold tabular-nums">{cardioBurn} kcal</span>
                               </div>
                               <div className="flex flex-col gap-1">
                                 <span className="text-white/20 uppercase tracking-wider font-bold">Total Duration</span>
                                 <span className="text-white/85 font-semibold tabular-nums">{totalDuration} mins</span>
                               </div>
                             </div>
                           </div>
                         );
                                    ...
                                    {archivedWorkouts.map((w) => {
                                      ...
                                      return (
                                        ...
                                        >
                                          ...
                                          <div className="flex items-center gap-3">
                                            <div className="text-[10px] text-gym-accent/70 font-bold tabular-nums">
                                              {getWorkoutCalories(w)} kcal
                                            </div>
                                            <div className="text-[10px] text-white/30 font-semibold tabular-nums">
                                              {w.exercisesCount} Ex.
                                            </div>
                                          </div>
                                        </div>
                                ...
                                <div className="flex items-center gap-8">
                                  ...
                                  <div>
                                    ...
                                    <div className="flex items-center gap-6">
                                      ...
                                      <div className="flex items-center gap-2">
                                        <Flame className="w-3 h-3 text-gym-accent" />
                                        <span className="text-[10px] text-white/50 uppercase tracking-widest font-bold">{getWorkoutCalories(workout)} kcal</span>
                                      </div>
                                    </div>
                                  </div>
                                </div>
                                  ...
                                  <div key={name} className="space-y-4">
                                    ...
                                    <div className="grid grid-cols-2 gap-2">
                                      {exerciseSets.map((s: any, idx: number) => {
                                        const isCardio = Object.values(POOLS).flat().some(e => e && e.name === name && e.pool === 'cardio');
                                        return (
                                          <div key={idx} className="bg-black/60 border border-white/10 p-3 flex items-center justify-between">
                                            <div className="flex flex-col">
                                              <span className="text-[7px] text-white/45 uppercase font-black">{isCardio ? 'Time' : 'Weight'}</span>
                                              <span className="text-sm font-semibold text-white/95 tabular-nums">{s.weight}{isCardio ? ' mins' : 'kg'}</span>
                                            </div>
                                        ...
                                        <div key={exName} className="flex justify-between items-start gap-4 pb-2 border-b border-white/5 last:border-0 last:pb-0">
                                          ...
                                          <div className="flex flex-wrap gap-1.5 justify-end">
                                            {exSets.map((set: any, idx: number) => (
                                              ...
                                              >
                                                {set.weight}{Object.values(POOLS).flat().some(e => e && e.name === exName && e.pool === 'cardio') ? ' mins × ' : 'kg × '}{set.reps}{Object.values(POOLS).flat().some(e => e && e.name === exName && e.pool === 'cardio') ? ' reps' : ''}
                                              </span>
                                            ))}
                                          </div>
                                        </div>
              ...
              <div className="bg-black/85 border border-white/20 rounded-lg p-8 space-y-8">
                <div>
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="pt-4 border-t border-white/5 grid grid-cols-1 sm:grid-cols-3 gap-6">
                      <div className="flex flex-col gap-2">
                         <span className="text-[9px] text-white/20 uppercase tracking-widest font-bold flex items-center gap-1.5">
                           <Ruler className="w-3 h-3 text-gym-accent/60" /> Height (cm)
                         </span>
                         <input 
                           type="number"
                           value={profile?.height || ""}
                           placeholder="Ex. 180"
                           className="bg-transparent border-b border-white/10 py-1 text-sm font-light focus:outline-none focus:border-gym-accent transition-all text-white placeholder-white/20"
                           onChange={(e) => {
                             const val = e.target.value === "" ? "" : parseFloat(e.target.value);
                             saveSettings({ height: val });
                             setProfile(prev => prev ? { ...prev, height: typeof val === "number" ? val : undefined } : null);
                           }}
                         />
                      </div>
                      <div className="flex flex-col gap-2">
                         <span className="text-[9px] text-white/20 uppercase tracking-widest font-bold flex items-center gap-1.5">
                           <Scale className="w-3 h-3 text-gym-accent/60" /> Weight (kg)
                         </span>
                         <input 
                           type="number"
                           ...
                           onChange={(e) => {
                             const val = e.target.value === "" ? "" : parseFloat(e.target.value);
                             saveSettings({ weight: val });
                             setProfile(prev => prev ? { ...prev, weight: typeof val === "number" ? val : undefined } : null);
                           }}
                         />
                      </div>
                      <div className="flex flex-col gap-2">
                         <span className="text-[9px] text-white/20 uppercase tracking-widest font-bold flex items-center gap-1.5">
                           <UserIcon className="w-3 h-3 text-gym-accent/60" /> Sex
                         </span>
                         <select 
                           value={profile?.sex || ""}
                           className="bg-transparent border-b border-white/10 py-1 text-sm font-light focus:outline-none focus:border-gym-accent transition-all text-white/85 [&>option]:bg-neutral-900 [&>option]:text-white cursor-pointer h-[26px]"
                           onChange={(e) => {
                             const val = e.target.value;
                             saveSettings({ sex: val });
                             setProfile(prev => prev ? { ...prev, sex: val } : null);
                           }}
                         >
            ...
            >
              ...
              {DAY_CONFIG.map((day, di) => (
                <div key={di} className="group">
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <span className="text-[10px] font-bold text-gym-accent uppercase tracking-[0.2em]">{day.label.replace(/day\s*/i, "")}</span>
                      <div className="flex items-center gap-3">
                        <h3 className="text-lg font-light italic font-serif text-white/90">{day.name}</h3>
                        <span className="text-[9px] text-white/10 px-2 py-0.5 border border-white/5 rounded-full uppercase tabular-nums">
                                          ...
                                          >
                                            <div className="flex items-center gap-2">
                                              ...
                                              <div className="flex items-baseline gap-1">
                                                ...
                                                <span className="text-[8px] text-white/40">{ex.pool === 'cardio' ? 'mins' : 'kg'}</span>
                                              </div>
                    ...
                    return (
                      <div key={poolKey} className="mb-8">
                        {equipmentGroups.map(group => {
                          const groupExercises = filtered.filter(e => e.equipmentCategory === group);
                          if (groupExercises.length === 0) return null;
            ...
            >
              ...
              <div className="p-8 space-y-4">
                <p className="text-xs text-white/60 leading-relaxed">
                  Select 1 of the {DAY_CONFIG.length} exercise day categories to categorize this routine. It will be saved under the corresponding section in your Routines tab:
                </p>
